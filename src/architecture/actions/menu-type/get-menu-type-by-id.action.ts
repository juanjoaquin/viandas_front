...
export async function getMenuTypeByIdAction(
    menuTypeId: string,
    ...
    if (!accessToken) {
        Logger.error(
            "[ACTION][GET-MENU-TYPE-BY-ID] Unauthorized — no access token",
    ...
    try {
        ...
        const result = await controller.getMenuTypeById(id);

        if (!result.success) {
            Logger.error(
                "[ACTION][GET-MENU-TYPE-BY-ID] Action returned error",
        ...
        Logger.error(
            "[ACTION][GET-MENU-TYPE-BY-ID] Unexpected error",
            error,
        );
