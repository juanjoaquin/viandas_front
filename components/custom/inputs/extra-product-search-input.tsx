...
type ExtraProductSearchInputProps = {
    value: string;
    onValueChange: (productId: string, product?: TExtraProduct) => void;
    selectedProduct?: Pick<TExtraProduct, "id" | "name"> | null;
...
export function ExtraProductSearchInput({
    value,
    onValueChange,
    selectedProduct,
    disabledIds,
...
}: ExtraProductSearchInputProps) {
    const listboxId = useId();
    const containerRef = useRef<HTMLDivElement>(null);
    const [query, setQuery] = useState("");
    const [results, setResults] = useState<TExtraProduct[]>([]);
            ...
            getAllExtraProductsAction({
                ...
                .then((result) => {
                    if (result.success) {
                        ...
                        setResults(activeOnly ? items.filter((p) => p.active) : items);
                    } else {
                        setResults([]);
                    }
                })
    ...
    function handleQueryChange(nextQuery: string) {
        ...
        if (value && nextQuery.trim() !== selectedProduct?.name) {
            onValueChange("");
        }
    }

    function handleSelectProduct(product: TExtraProduct) {
                ...
                >
                    {isLoading ? (
                        ...
                        results.map((product) => {
                            ...
                            const isDisabled = disabledIds?.has(product.id) ?? false;
